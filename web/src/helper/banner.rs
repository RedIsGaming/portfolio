...
pub fn Banner() -> impl IntoView {
    ...
    view! {
        <section class="relative">
            <VideoControls />
            <article class="section-nodefault !bg-transparent px-0 flex-col items-center absolute inset-0">
                <img src="./assets/Red.webp" class="border-red-600 border-2 rounded-full w-28 sm:w-32 h-28 sm:h-32 flex
                    ...
                    alt={title.clone()} 
                />
                <h1 class="text-7xl sm:text-8xl md:text-9xl font-mono font-bold w-auto pt-4 text-white">{title}</h1>
